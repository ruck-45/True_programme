...
int BFS(int **M,struct list **L,int v,int a,int *vis,int *par)
{
...
void Check_Cycles(int **M,struct list **L,int v)
{
	int *vis,*par,i;
	...
	for(i=0;i<v;i++)
		if(vis[i]==0)
			if(BFS(M,L,v,i,vis,par))
			{
				printf("\n\nCycles found ! The graph is cyclic.");
				return;
			}
			
	printf("\n\nNo cycles found! The graph is Acyclic");
}
...
void main()
{
	...
	printf("\nEnter 2 to Check graph is cyclic or not");
	printf("\nEnter Choice :");
	scanf("%d",&choice);
